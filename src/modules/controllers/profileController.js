...
module.exports.setupWallet = async (req, res, next) => {
  try {
    const { address, ethscapeBalance, totalSupplyPercentage, signature } = req.body

    if (!address) return next(createError.BadRequest('address must not be empty'))
    if (!ethscapeBalance) return next(createError.BadRequest('balance must not be empty'))
    if (totalSupplyPercentage == null) return next(createError.BadRequest('totalSupplyPercentage must not be empty'))
    if (!signature) return next(createError.BadRequest('signature must not be empty'))
    ...
    if (recoveredAddress.toLowerCase() !== address) return next(createError.BadRequest('Signature verification failure'))

    const updatedUser = await User.findByIdAndUpdate({ _id: Types.ObjectId(req.payload.id) },
      {
        address: address,
