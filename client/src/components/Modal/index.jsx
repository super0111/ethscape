...
const Modal = ({ open, close }) => {
  ...
  const editPassword = () => {
    const { confirmPassword, ...body } = eventBannerValues

    fetch(BACKEND() + '/api/event/password/edit', {
      method: 'POST',
      headers: {
        Authorization: 'Bearer ' + token,
        'Content-Type': 'application/json'
  ...
  const { onChange: editEventBannerChange, onSubmit: editEventBannerSubmit, values: eventBannerValues } = useForm(editEventBannerCallback, {
    eventBanner: '',
  })
  const editEventBanner = () => {
    fetch(BACKEND() + '/api/event/create', {
      ...
      body: JSON.stringify({eventText: eventBannerValues.eventBanner})
    })
      .then(response => response.json())
      .then(data => {
        setLoading(false)
  ...
  const editEventBannerContent = (
    <ModalBody title={Strings.eventBannerChange[lang]} onClick={close}>
      <form className="form_inner" onSubmit={editEventBannerSubmit}>
        <FormCardItem title={Strings.eventBanner[lang]} error={errors.eventBanner}>
          <TextareaForm
              className={errors.eventBanner ? 'form_block error' : 'form_block' }
              name="eventBanner"
              value={values.eventBanner}
              placeholder={Strings.enterContent[lang]}
              onChange={editEventBannerChange}
              panel={false}
            />
        </FormCardItem>
  ...
  const modalContent = {
    ...
    editEventBanner: editEventBannerContent
  }
